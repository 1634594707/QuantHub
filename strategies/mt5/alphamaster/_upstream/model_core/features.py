...
class MT5FeatureEngineer:
    ...
    @staticmethod
    def _atr(
        close: torch.Tensor, high: torch.Tensor, low: torch.Tensor, w: int = 14
    ) -> torch.Tensor:
        pc = torch.cat([close[:, :1], close[:, :-1]], dim=1)
        tr = (
            torch.stack([high - low, (high - pc).abs(), (low - pc).abs()], dim=-1)
            .max(dim=-1)
            .values
        )
    ...
    def _robust_norm(x: torch.Tensor, w: int = 200) -> torch.Tensor:
        ...
        out = torch.clamp(
            (x32 - med) / mad, -MT5FeatureEngineer._CLIP_BOUND, MT5FeatureEngineer._CLIP_BOUND
        )
    ...
    @staticmethod
    def _typical_dev(
        close: torch.Tensor, high: torch.Tensor, low: torch.Tensor, w: int = 20
    ) -> torch.Tensor:
    ...
    @classmethod
    def _ult_osc(cls, close: torch.Tensor, high: torch.Tensor, low: torch.Tensor) -> torch.Tensor:
    ...
    def _c_rs_vol(cls, raw: dict) -> torch.Tensor:
        ...
        rs_bar = torch.log((high + eps) / (close + eps)) * torch.log(
            (high + eps) / (open_ + eps)
        ) + torch.log((low + eps) / (close + eps)) * torch.log((low + eps) / (open_ + eps))
    ...
    def _c_ret_accel(cls, raw: dict) -> torch.Tensor:
        ...
        ret5_raw = torch.log(close[:, 5:] / (close[:, :-5] + eps))
        ret5 = torch.cat(
            [torch.zeros(N, 5, device=close.device, dtype=close.dtype), ret5_raw], dim=1
        )
    ...
    def _c_amihud_illiq(cls, raw: dict) -> torch.Tensor:
        ...
        log_ret = torch.cat(
            [torch.zeros(N, 1, device=close.device, dtype=close.dtype), log_ret_raw], dim=1
        )
    ...
    def _c_dmi_adx_14(cls, raw: dict) -> torch.Tensor:
        ...
        tr = (
            torch.stack([high - low, (high - prev_c).abs(), (low - prev_c).abs()], dim=-1)
            .max(dim=-1)
            .values
        )  # [N, T]
    ...
    def _c_cs_rank_ret5(cls, raw: dict) -> torch.Tensor:
        ...
        ranks.scatter_(
            0,
            order,
            torch.arange(N, dtype=ret5.dtype, device=ret5.device).unsqueeze(1).expand(N, T),
        )
    ...
    def _c_cs_zscore_ret20(cls, raw: dict) -> torch.Tensor:
        ...
        ret20_raw = torch.log(close[:, 20:] / (close[:, :-20] + eps))
        ret20 = torch.cat(
            [torch.zeros(N, 20, device=close.device, dtype=close.dtype), ret20_raw], dim=1
        )  # [N, T]
        if N == 1:
...
for _name, _category, _compute in _FEATURE_DEFS:
    if _ACTIVE_FEATURES is not None and _name not in _ACTIVE_FEATURES:
        continue
    FEATURE_REGISTRY.register_feature(FeatureSpec(name=_name, category=_category, compute=_compute))
