...
_UNIVERSE_ALIAS: dict[str, str] = {
    "hs300": "000300", "沪深300": "000300",
    "sz50": "000016", "上证50": "000016",
    "zz500": "000905", "中证500": "000905",
    "zz1000": "000852", "中证1000": "000852",
}
...
def _score_short(df: pd.DataFrame, fund_flow: dict | None = None) -> dict | None:
    ...
    details['kdj'] = {
        ...
        'k': kdj_result['k'], 'd': kdj_result['d'], 'j': kdj_result['j'],
        'signal': kdj_signal,
    ...
    details['macd'] = {
        'score': max(0, macd_score),
        'dif': macd_result['dif'], 'dea': macd_result['dea'],
    ...
    details['bollinger'] = {
        'score': max(0, boll_score),
        'upper': boll_result['upper'], 'middle': boll_result['middle'], 'lower': boll_result['lower'],
...
class SelectorStrategy(StrategyBase):
    ...
    def produce(
        ...
        **kwargs: Any,
        ...
        logger.info("选股扫描: universe=%s term=%s codes=%d top_n=%d",
                    universe, term, len(codes), top_n)
