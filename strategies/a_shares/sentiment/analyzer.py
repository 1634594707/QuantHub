...
_NEGATION_WORDS = [
    '不', '没有', '无', '未', '并非', '绝非', '不是', '不会',
    '不可能', '难以', '不足', '不佳', '不利', '不愿', '未能',
    '不要', '别', '莫', '毋', '勿', '不必', '尚未',
]

# 程度副词（增强/减弱情感强度），源自原 core/sentiment.py
_INTENSIFIERS = {
    '非常': 1.5, '极其': 2.0, '十分': 1.5, '极度': 2.0,
    '较为': 0.7, '略微': 0.5, '稍微': 0.5, '有点': 0.6,
    '明显': 1.3, '显著': 1.3, '大幅': 1.5, '持续': 1.2,
    '强烈': 1.8, '高度': 1.5, '特别': 1.4,
}
...
_POSITIVE_KEYWORDS = [
    ...
    '龙头', '白马', '绩优', '朝阳', '景气', '反转',
    '矿价上涨', '涨价', '供应紧缺', '供不应求', '产能扩张',
    '库存下降', '需求旺盛', '价格上涨', '价格反弹',
    '资源注入', '储量增加', '品位提升', '满产', '扩产',
    '勘探突破', '采矿权获批',
    '营收大增', '利润大增', '毛利率提升', '现金流改善',
    '负债率下降', '降本增效', '订单饱满', '开工率提升',
    '抄底', '拉涨', '探底回升', '绝地反击', '触底反弹',
    '资金流入', '主力买入', '北向资金', '机构增持',
]
...
_NEGATIVE_KEYWORDS = [
    ...
    '预警', '预亏', '戴帽', '召回',
    '矿价下跌', '跌价', '供应过剩', '产能过剩', '库存积压',
    '需求疲软', '需求下滑', '价格下跌', '价格暴跌',
    '停产', '减产', '关停', '安全事故', '矿难',
    '品位下降', '资源枯竭', '开采成本上升',
    '营收下滑', '利润暴跌', '毛利率下降', '现金流紧张',
    '负债率高', '债务违约', '坏账', '计提减值',
    '停工', '裁员', '降薪', '欠薪',
    '资金流出', '主力卖出', '主力出逃', '资金出逃',
    '破位', '破发', '恐慌', '踩踏', '杀跌',
...
class SentimentAnalyzer:
    ...
    def _context(text: str, idx: int) -> tuple[bool, float]:
        """检查关键词前缀是否含否定词/程度副词。"""
        negated = any(
            text.rfind(neg, max(0, idx - 10), idx) != -1
