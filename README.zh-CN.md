...
- **多市场统一工作台**：统一管理 A 股、美股、加密资产和 MT5 数据及策略；每个市场只调用配置的 primary，供应商不可用时返回明确错误，不跨源回退。
...
- 总览账户口径、历史研究结果复用和状态后的下一步动作仍需按[用户体验、分类与分析报告改进需求](docs/PRODUCT_USABILITY_AND_REPORT_REQUIREMENTS.md)执行整改；自选和持仓以服务端为真源，不使用离线业务缓存回退。
...
| `configs/us_stocks.yaml` | 美股 primary 数据源与显式诊断源配置 |
