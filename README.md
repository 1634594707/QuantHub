...
![Version](https://img.shields.io/badge/version-v0.1.1-4FB3C8)
...
`v0.1.1` 将综合评估、因子研究和 AI 复核整理为一条可追溯、可恢复的研究链路，重点解决入口分散、结果难找和首次使用门槛高的问题。

- **工作台一键评估**：进入单标的工作台后可直接启动均衡综合评估，运行时展示准备数据、量化快照、新闻 AI、价格结构 AI、模型共识和报告生成进度；移动端提供完整入口。
- **因子研究闭环**：14 个候选因子经过训练隔离、样本外 IC、成本后回测和回撤验证；研究参数与结果自动保存，支持历史恢复和同标的对比。
- **回撤与尾部风险提示**：统一展示当前回撤、最大回撤、VaR、CVaR、Calmar 和五级回撤动作，明确风险、失效条件与下一步实验。
- **受约束的 AI 科研复核**：AI 读取已锁定的结构化快照，审阅过拟合、样本量、IC 衰减、成本敏感度和市场状态依赖，不改写量化状态。
- **新手渐进式阅读**：新增研究目的与周期模板、四类示例报告、六项术语解释、首次阅读顺序、统一错误说明和本机匿名易用性指标。
- **统一结果回链**：研究任务、自动化、提醒、信号、故障和全局检索共享结果定位规则，关联产出最多一次点击返回研究正文。

完整变更与升级说明见 [v0.1.1 发布说明](docs/releases/v0.1.1.md) 和 [研究工作流与因子验证更新](docs/posts/2026-07-30-research-factor-update.md)。

## 为什么使用 QuantHub

- **单一综合评估入口**：工作台支持一键启动量化快照、新闻 AI、价格结构 AI 和模型共识，并将结果写入同一份研究记录。
- **严格且易读的因子验证**：使用训练、隔离和样本外区间评估 14 个趋势、反转、量价与风险因子；结果自动保存、可回看和对比，并提供周期模板、稳健/趋势/回撤研究模板、示例报告、术语解释、首次阅读引导和可展开专业详情。高级 AI 只受约束地复核过拟合、状态依赖和后续实验。
...
`v0.1.1` 加入工作台一键评估、因子样本外验证、回撤动作、风险指标、AI 科研复核、研究结果闭环、新手渐进式学习、统一结果回链、美股 Yahoo 历史数据、多供应商 AI 设置、PA 输出质量闸门、K 线引用语义校验和闭合交易质量分析。完整说明见 [v0.1.1 发布说明](docs/releases/v0.1.1.md)，易用性实施状态见 [新手易用性路线图](BEGINNER_USABILITY_ROADMAP.md)，PA 参考适配见 [PA Agent 能力审计与适配](docs/PA_AGENT_ADAPTATION.md)，长期职责边界见 [功能边界](docs/FUNCTION_BOUNDARIES.md)。
