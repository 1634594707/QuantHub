...
- **AI 输出质量闸门**：PA 两阶段结果经过结构、概率、终局和交易几何校验，可修复错误只重试一次，未通过时禁止发布信号。
...
本次研究工作流更新加入了因子样本外验证、回撤动作、风险指标、美股 Yahoo 历史数据、多供应商 AI 设置、PA 输出质量闸门和闭合交易质量分析。完整说明见 [QuantHub 研究工作流与因子验证更新](docs/posts/2026-07-30-research-factor-update.md)，PA 参考适配见 [PA Agent 能力审计与适配](docs/PA_AGENT_ADAPTATION.md)，长期职责边界见 [功能边界](docs/FUNCTION_BOUNDARIES.md)。
